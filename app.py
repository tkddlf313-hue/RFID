...
with st.sidebar:
    ...
    st.warning(
        "⚠️ **클라우드 배포(Streamlit Cloud) 안내**:\n\n"
        "클라우드 환경에서는 PC에 직접 연결된 **로컬 RFID 리더기(COM 포트)**에 접근할 수 없습니다.\n\n"
        "실제 장비 테스트는 **로컬 환경(실행.bat)**에서 구동해야 합니다. "
        "클라우드에서는 **모의 스캔(시뮬레이션)** 및 **TCP/IP** 방식을 사용해 주세요."
    )
...
with tab1:
    ...
    if submitted:
        if not item_name.strip() or not lot_number.strip():
            st.error("품명과 Lot 번호는 필수입니다.")
        else:
            is_valid = True
            if epc_mode == "직접 입력":
